    ...
    fn rpa3_fixture_byte_perfect_extraction() {
        ...
        for e in &listed.entries {
            eprintln!(
                ...
                e.offset.get(),
                e.length.get(),
                e.prefix.as_ref().map(|p| p.len()).unwrap_or(0)
            );
        }
        let sample = listed
            .entries
            .iter()
            .find(|e| e.path == "greeting.txt")
            .expect("greeting.txt missing");
        let bytes = read_entry(&archive, sample).expect("read");
        assert_eq!(bytes, b"hello renpyex!\n");
        assert_eq!(sha256(&bytes), crate::verify::sha::sha256(&bytes));
    }
}
